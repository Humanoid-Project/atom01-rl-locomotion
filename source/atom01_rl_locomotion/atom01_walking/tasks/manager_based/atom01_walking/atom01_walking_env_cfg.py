...
from isaaclab_tasks.manager_based.locomotion.velocity.mdp import rewards as loco_rewards
...
class CommandsCfg:
    base_velocity = UniformVelocityCommandCfg(
        asset_name="robot",
        resampling_time_range=(10.0, 10.0), # 몇 초마다 명령이 바뀔지
        ...
        ranges=UniformVelocityCommandCfg.Ranges(
            ...
            ang_vel_z=(-0.5, 0.5), # yaw 회전 속도 범위
        ),
    )

@configclass
class Atom01WalkingSceneCfg(InteractiveSceneCfg):
...
@configclass
class Atom01WalkingEnvCfg(ManagerBasedRLEnvCfg):
    commands: CommandsCfg = CommandsCfg()

    scene: Atom01WalkingSceneCfg = Atom01WalkingSceneCfg(
